...
# t-tests ----
desc_vars <- c("pop", "popdensity", "pctnhblack", "pctnbmin", "pctnhblack_total", "pctnbmin_total", "more_white", "pctowneroccupied", "mhmval", "hinc", "pctpov", "pctblackpov", "pctnbminpov", "pctownerocc_total", "pcthincjobs_total", "pctincopp_total", "annexing", "underbound_black", "underbound_nbmin", "more_white_1")
...
write_csv(df, paste0(savedir, "diff_pval.csv"))

rm(df, diff, post, post_0, post_1, post_difference, pre_0, pre_1, pre_difference, pval)

# description of population and area annexed ----
panel_annual %>%
  filter(!time %in% "2000 to 2007" & annexing == 1) %>%
  group_by(vra, post) %>%
  summarize(pop_total = mean(pop_total_1, na.rm = T),
            area_total = mean(area_total_1, na.rm = T))
...
ggsave(filename = paste0(savedir, "PTA_annual.pdf"),
       plot = gg, 
       dpi = 300)
