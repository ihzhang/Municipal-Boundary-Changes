...
cdps07 <- read_csv("pl2007_cleaned.csv") %>% # want to know which places are CDPs
  select(Geo_NAME, plid) %>%
  mutate(cdp = ifelse(grepl("CDP|cdp", Geo_NAME), 1, 0)) %>%
  filter(cdp==1)
blocks2007 %<>% # filter out places in 2007 that are CDPs
...
blkids <- Reduce(intersect, list(unique(blocks2007$blkid), unique(blocks2013_pl$blkid))) # find only blocks common to each other because the block had to exist in 2007 and in 2013 to be annexable and annexed 
blocks2007 %<>%
  filter(blkid %in% blkids) %>%
  filter(!duplicated(blkid))
blocks2013_pl %<>%
