...
# buffers ----
# 2007 ----
cdps <- read_csv("pl2007_cleaned.csv") %>%
  select(Geo_NAME, plid) %>%
  mutate(cdp = ifelse(grepl("CDP|cdp", Geo_NAME), 1, 0)) %>%
  filter(cdp==1)
get_buffers_07 <- function(state_code) {
  blocks <- st_read(paste0("shapefiles/2007/blocks/states/", state_code, "_allblocks.shp")) %>%
    st_transform(., 3488)
  blocks %<>%
  ...
  plid_list <- read_csv(file = paste0("spatial_files/2007/", state_code, "_block_plids_2007blk-2007pl_90pct.csv"))
  plid_list %<>% 
    select(blkid, plid) %>%
