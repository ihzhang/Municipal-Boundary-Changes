...
state_list_2000_rac <- list.files("Demography/LODES data/2000/rac/", all.files = FALSE, full.names = FALSE) # read in all files, saves file names in a vector
file_list_2000_rac <- list() # initiate an empty list
for (i in 1:length(state_list_2000_rac)) {
  file_list_2000_rac[[i]] <- read_csv(file = paste0("Demography/LODES data/2000/rac/", state_list_2000_rac[[i]])) %>%
    mutate(njobs00 = C000,
           nhincjobs00 = CE03) %>%
...
rm(file_list_2000_rac, state_list_2000_rac) # I like to remove useless objects from the environment ASAP to avoid clutter
rac_2000$Year <- 2000
write_csv(rac_2000, file = "rac_2000.csv")
rm(rac_2000)
...
# WAC ####
# 2014 ####
state_list_2014_wac <- list.files("LODES data/2014/wac/", all.files = FALSE, full.names = FALSE) # read in all files, saves file names in a vector
file_list_2014_wac <- list() # initiate an empty list
for (i in 1:length(state_list_2014_wac)) {
  file_list_2014_wac[[i]] <- read_csv(file = paste0("LODES data/2014/wac/", state_list_2014_wac[[i]])) %>%
    mutate(jobs = C000,
           man = CNS05,
           ret = CNS07) %>%
...
wac_2014$Year <- 2014
write_csv(wac_2014, file = "wac_2014.csv")
...
# turn list into txt file - a good place to double-check that the list has 49 observations
lapply(url_list, cat, "\n", file="LODES_wac_00.txt", append=TRUE)
rm(url_list)

# code 
# wget -P Google\ Drive/My\ Drive/Stanford/QE2/Demography/LODES\ data/2000/rac -i Google\ Drive/My\ Drive/Stanford/QE2/LODES_00.txt

state_list_2000_wac <- list.files("LODES data/2000/wac/", all.files = FALSE, full.names = FALSE) # read in all files, saves file names in a vector
file_list_2000_wac <- list() # initiate an empty list
for (i in 1:length(state_list_2000_wac)) {
  file_list_2000_wac[[i]] <- read_csv(file = paste0("LODES data/2000/wac/", state_list_2000_wac[[i]])) %>%
    mutate(jobs = C000,
           man = CNS05,
           ret = CNS07) %>%
    select(w_geocode, jobs, man, ret)
...
rm(file_list_2000_wac) # I like to remove useless objects from the environment ASAP to avoid clutter
wac_2000$Year <- 2000
...
# turn list into txt file - a good place to double-check that the list has 49 observations
lapply(url_list, cat, "\n", file="LODES_wac_07.txt", append=TRUE)
rm(url_list)
...
# clean 2007 data ####
state_list_2007_wac <- list.files("Demography/LODES data/2007/wac/", all.files = FALSE, full.names = FALSE) # read in all files, saves file names in a vector
file_list_2007_wac <- list() # initiate an empty list
for (i in 1:length(state_list_2007_wac)) {
  file_list_2007_wac[[i]] <- read_csv(file = paste0("Demography/LODES data/2007/wac/", state_list_2007_wac[[i]])) %>%
    mutate(njobs07 = C000,
           nhincjobs07 = CE03) %>%
    select(h_geocode, njobs07, nhincjobs07)
}
...
rm(file_list_2007_wac, state_list_2007_wac) # I like to remove useless objects from the environment ASAP to avoid clutter
wac_2007$Year <- 2007
write_csv(wac_2007, file = "wac_2007.csv")
rm(wac_2007)

# clean 2017 data ####
state_list_2007_wac <- list.files("LODES data/2007/wac/", all.files = FALSE, full.names = FALSE) # read in all files, saves file names in a vector
file_list_2007_wac <- list() # initiate an empty list
for (i in 1:length(state_list_2007_wac)) {
  file_list_2007_wac[[i]] <- read_csv(file = paste0("LODES data/2007/wac/", state_list_2007_wac[[i]])) %>%
    mutate(jobs = C000,
           man = CNS05,
           ret = CNS07) %>%
    select(w_geocode, jobs, man, ret)
